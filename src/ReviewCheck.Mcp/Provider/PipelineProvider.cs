...
/// <summary>
/// The REAL provider (docs/24 §7): local git diff → deterministic pipeline → narrated blocks.
...
public sealed class PipelineProvider(IDiffReader diffReader, AnalysisPipeline pipeline) : IReviewProvider
{
    public Task<AnalyzedReview> AnalyzeAsync(Source source)
    {
        ...
        var blocks = structural.Blocks
            .Select(b => BlockGuard.Ensure(StructuralNarrator.ToBlock(b)))
            .ToList();

        var estimated = blocks.Sum(b => b.EstimatedMinutes ?? 0);
        return Task.FromResult(new AnalyzedReview(
            structural.Title,
            blocks,
