...
///
/// Deliberately FOUR methods, no more: there is no per-block "post one comment now" method. Every
/// comment collected during a review is batched into ONE <see cref="SubmitReviewAsync"/> call, made
/// only when the caller has an explicit <c>confirm:true</c> in hand (GUARDRAILS G8) — nothing in this
/// interface lets a caller post incrementally, so "post before the human confirms" is not a discipline
/// to remember, it is impossible to express against this seam.
/// </summary>
