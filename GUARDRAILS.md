...
| G8 | **No posting to a platform without confirmation** | `IPullRequestPlatform.SubmitReviewAsync` is the ONLY posting call on the interface — there is no per-block "post now" method, so a PR review is always ONE batched call. `ReviewEngine.SubmitReviewAsync` makes that call only when the caller passes `confirm:true`; without it, `submit_review` returns the same outcome as a preview (`posted:false`) and touches the network not at all | **Deterministic** |
| G9 | **Posted comments are the reviewer's own words** | Each `PullRequestComment.Body` sent to the platform is built exclusively from that block's `request_correction` note — the LLM's `what`/`why`/`link` narration never reaches a posted comment | **Deterministic** |
| G10 | **No self-approval on a PR** | `is_self_review` is computed **once**, when the review opens — comparing the authenticated token identity to the PR's author (`IPullRequestPlatform.GetSummaryAsync`/`ListAsync`) — and carried in session state for the review's whole lifetime. `list_pull_requests` excludes self-authored PRs outright; `get_review_plan` surfaces the flag so the agent discloses it **before the first block**, not as a surprise at close time. `ReviewEngine.SubmitReviewAsync` then enforces it at close: when `is_self_review` is true (or unresolved — fails **closed**), the review event is forced to `Comment`; `Approve`/`RequestChanges` are never even constructed, so a caller that ignored the earlier disclosure still can't reach them. | **Deterministic** (both detection and the outcome gate are unconditional code, not an instruction) |
